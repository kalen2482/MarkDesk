...
<p align="center">
  <strong>轻量、本地优先的 Markdown 编辑器。</strong><br />
  编写 Markdown · 直接编辑预览内容 · 文件始终保存在本机。
</p>
...
Markdown 开放、便携，适合长期保存；但只编辑源码，有时不够直观。MarkDesk 同时提供源码编辑和可视化编辑：需要精确控制时编辑 Markdown，需要排版和检查效果时直接在预览中修改。

| ✍️ 两种编辑方式 | 🔒 文件只属于你 | 🪟 跨平台桌面应用 |
| --- | --- | --- |
| 源码、分栏、可视化三种模式，可按需要随时切换。 | 无需账号、无云同步，不在服务端处理文档。 | 基于 Electron，可在 Windows、macOS 和 Linux 上构建。 |

## 🚀 核心特性

- **双向编辑**：既可编辑 Markdown 源码，也可直接编辑预览内容，修改会同步回 Markdown。
- **长文档导航**：根据标题实时生成目录，点击即可定位到对应章节。
- **Markdown 扩展支持**：支持 CommonMark、GFM 表格与任务列表、代码高亮、KaTeX、Mermaid、提示块、脚注、高亮、下划线和 `[TOC]`。
- **本地图片**：通过系统文件选择器插入图片，并支持显示 Markdown 中的相对路径图片。
- **写作体验**：提供深浅主题、专注模式、快捷键、查找替换、字数统计和滚动同步。
- **文件关联**：Windows 安装包可关联 `.md`、`.markdown`、`.mdx` 文件类型。

## ⚡ 快速开始

### 安装 Windows 应用

从项目 Releases 下载并运行最新的 Windows 安装包。安装完成后，可以在文件资源管理器中直接使用 MarkDesk 打开 Markdown 文件。
...
MarkDesk 基于 Electron，同一份源代码可在 macOS 和 Linux 上构建。安装 Node.js 18+ 后运行：
...
本项目在开发过程中使用了 AI 辅助工具。产品方向、代码审查、构建测试和最终发布均由维护者负责。
